...
def parse_args():
    p = argparse.ArgumentParser(description="Train/Eval Xception on images")
    p.add_argument("--data_dir", type=str, required=True)
    p.add_argument("--out_dir", type=str, required=True)
    p.add_argument("--epochs", type=int, default=8)
    p.add_argument("--batch_size", type=int, default=24)
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--freeze_until", type=int, default=80)
...
def main():
    ...
    # Xception requires 299x299
    train_ds, val_ds, test_ds = build_image_datasets(args.data_dir, image_size=(299, 299), batch_size=args.batch_size)

    model = build_xception((299, 299, 3), freeze_until=args.freeze_until)
    model.compile(
