...
from src.models import build_video_lstm
from src.utils import build_video_dataset


def parse_args():
    p = argparse.ArgumentParser(description="Train/Eval CNN+LSTM on videos")
...
def main():
    ...
    train_ds = build_video_dataset(args.data_dir, split="train", num_frames=args.num_frames, image_size=(224, 224), batch_size=args.batch_size)
    val_ds = build_video_dataset(args.data_dir, split="val", num_frames=args.num_frames, image_size=(224, 224), batch_size=args.batch_size, shuffle=False)
    test_ds = build_video_dataset(args.data_dir, split="test", num_frames=args.num_frames, image_size=(224, 224), batch_size=args.batch_size, shuffle=False)
